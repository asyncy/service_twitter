...
def followers():
    t = getOauth()
    data = body()
    if (data.get("user_id") is None or data.get("user_id") == "") and (
        data.get("screen_name") is None or data.get("screen_name") == ""
    ):
        res = {
            "status": "failed",
...
def stream_subscribe():
    ...
    if data["data"].get("track") is None or data["data"].get("track") == "":
        res = {"status": "failed", "reason": "'track' parameter is missing"}
        response_code = 400
        return dumps(res), response_code
